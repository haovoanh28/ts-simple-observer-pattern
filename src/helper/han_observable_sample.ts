...
export function appendObservable<T extends Dictionary<any>>(obj: T, key: string) {
  const __propObserverIdSetMap__ = obj.__propObserverIdSetMap__;
  const storage = obj.__storage__;
  storage[key] = {};
  Object.keys(obj).forEach(key => {
    if (!__propObserverIdSetMap__[key]) {
      makeObservableData(obj, obj, key, storage, __propObserverIdSetMap__, false);
    }
  });
}
